...
def climbingLeaderboard(ranked, player):
    checkpoints = sorted(list(set(ranked)))[::-1]
    player_rankings = []

    for score in player:
        rank = get_rank(checkpoints, score)
        # print('MAP', score, rank, checkpoints)
...
scores = [int(x) for x in textwrap.dedent(raw_players).split(' ')]
ranking_refs = [int(x) for x in textwrap.dedent(raw_ranking_scores).split(' ')]
rankings = [int(x) for x in textwrap.dedent(raw_rankings).split(' ')]

out_rankings = climbingLeaderboard(ranking_refs, [295])
print(out_rankings)
# print(rankings)
# assert rankings == out_rankings